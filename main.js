  ...
  try {
    ...
    if (Array.isArray(rpcData)) {
      ...
      rpcData.forEach(rpc => {
        ...
        rpcObj[rpc.name.toLowerCase()] = rpc.rpcUrl;
...
// Perintah /listchain - Tampilkan hanya nama jaringan
bot.onText(/^\/listchain$/, (msg) => {
  ...
  const networks = displayData.map(net => `🟢 ${net.name}`).join('\n');
...
bot.onText(/^\/estimate$/, async (msg) => {
  ...
  try {
    ...
    // Tangani error ENS secara khusus
    if (e.message && e.message.includes("ENS")) {
      bot.sendMessage(msg.chat.id, `❌ Gagal estimasi: Jaringan ini tidak mendukung ENS. Coba gunakan alamat kontrak langsung.`);
    } else {
...
// Perintah /network - Perbaikan untuk mendukung huruf kapital
bot.onText(/^\/network\s+(\w+)$/, async (msg, match) => {
  ...
  if (rpcUrls[networkName]) {
    ...
    try {
      ...
      // Cari nama asli untuk tampilan
      const displayName = rpcUrls._displayData ? 
        rpcUrls._displayData.find(d => d.name.toLowerCase() === networkName)?.name || networkName 
        : networkName;
    ...
    bot.sendMessage(msg.chat.id, `❌ Jaringan "${match[1]}" tidak ditemukan.\n\n📋 Jaringan tersedia: ${rpcUrls._displayData ? rpcUrls._displayData.map(d => d.name).join(', ') : Object.keys(rpcUrls).filter(k => k !== '_displayData').join(', ')}`);
  }
});
